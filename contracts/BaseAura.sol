 ...
 * Each address can only mint one NFT, and the aura can be updated
 */
contract BaseAura is ERC721, ERC721URIStorage, Ownable {
    using Strings for uint256;

    // Token ID counter
    uint256 private _nextTokenId;

    // Mapping from address to token ID (for checking if address already has NFT)
    ...
    // Events
    event AuraMinted(address indexed owner, uint256 indexed tokenId, string auraType);
    ...
    /**
     * @dev Mint a new Base Aura NFT
     * @param auraType The type of aura (fire, water, tide, rock)
     */
    function mint(string memory auraType) public {
        require(!_hasMinted[msg.sender], "Address already has an Aura NFT");
        ...
        emit AuraMinted(msg.sender, tokenId, auraType);
    }
    ...
    function getTokenByAddress(address owner) public view returns (uint256) {
        require(_hasMinted[owner], "Address has no Aura NFT");
